...
/**
 * DTO Usuario segun contrato (usado en /login y /usuarios).
 *
 * Nota:
 * - `idPersona` (id_persona) es el ID recomendado para rutas como /usuarios/{id}/notificaciones.
 * - `mongoId` (_id) se mantiene por compatibilidad cuando el backend lo envia.
 * - Algunos backends envian el ObjectId como `_id`, otros como `id` o `userId`.
 */
